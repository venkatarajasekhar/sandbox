...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    dim = 2 * ( N + num_landmarks)   # <-- * 2 for X and Y
    ...
    Xi = matrix()
    Xi.zero(dim, 1)
    Xi.value[0][0] = world_size / 2.0    # <-- initial position
    Xi.value[1][0] = world_size / 2.0

    #Process the data
    for k in range(len(data)):
        # n is the index of the robot pose in the matric/vector
        n = k * 2        # <-- times 2 because 2D X and Y in same matrix
                         # <-- better than X, Y matrix because can take covariance 

        measurement = data[k][0]         # <-- extract measurement and motion from data
        motion = data[k][1]

        #integrate the measurements
        for i in range(len(measurement)):
            # m is the index of the landmark coordinate in the matric/vector
        ...
        # update the infromation matrix/vector based on the robot motion
        for b in range(4):
