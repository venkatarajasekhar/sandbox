...
class robot:
    ...
    def move(self, motion):
        # ADD CODE HERE
        alpha = motion[0]
        a_noise = random.gauss(0, self.steering_noise)
        alpha += a_noise
        ...
        if (abs(beta) > 0.01) :
            ...
            theta = self.orientation
            _x = self.x + d * cos(theta)
            _y = self.y + d * sin(theta)
            _theta = theta
    ...
    def sense(self, flag = 1): #do not change the name of this function
        ...
        for landmark in landmarks:
            ...
            #Z.append(atan2(delta_x, delta_y) - theta )
            noise = random.gauss(0, self.bearing_noise)
            if flag == 0 :
                Z.append((atan2(delta_y, delta_x) - theta) % (2*pi) )
            else:
                Z.append((atan2(delta_y, delta_x) - theta + noise) % (2*pi) )
...
def particle_filter(motions, measurements, N=500): # I know it's tempting, but don't change N!
    ...
    for i in range(N):
        ...
        #print r.bearing_noise
        #print r.steering_noise
        #print r.distance_noise
        p.append(r)
    ...
    for t in range(len(motions)):
        ...
        w = []
        for i in range(N):
            #print p[i].bearing_noise
            #print p[i].steering_noise
            #print p[i].distance_noise
