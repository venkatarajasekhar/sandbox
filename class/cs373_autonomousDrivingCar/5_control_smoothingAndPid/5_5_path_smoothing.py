...
#def smooth(path, weight_data = 0.0, weight_smooth = 0.1, tolerance = 0.000001):
    ...
    change = tolerance	                                     # Change in path value in 1 iteration
                                                             # Change is computed on the entire path at once!

    while change >= tolerance:                               #Unlimited nb of loop, until path stabilize
        change = 0.0
        for i in range(1, len(path)-1):                      # Take all, but first and last
            for j in range(len(path[0])):                    # j: number of dimensions
                aux = newpath[i][j]
                newpath[i][j] += weight_data * (path[i][j] - newpath[i][j])
                newpath[i][j] += weight_smooth * (newpath[i-1][j] + newpath[i+1][j] - (2.0 * newpath[i][j]))
                change += abs(aux - newpath[i][j])           # contribution to change by this value
